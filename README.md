...
### 1. 起動の手順
1. ダウンロードした ZIP ファイルを右クリックし、「すべて展開...」を選択して解凍します。
2. 解凍されたフォルダを開き、**`MDPythonPatternMaker.WPF.exe`** という名前のファイルを探します。
   - ※ 設定によっては `.exe` が表示されず、アイコンと名前だけが見える場合があります。
   - 下記のようなアイコンのファイルが実行プログラムです。
3. `MDPythonPatternMaker.WPF.exe` をダブルクリックして起動します。

![起動ファイルの確認](./docs/launch_guide.png) 
